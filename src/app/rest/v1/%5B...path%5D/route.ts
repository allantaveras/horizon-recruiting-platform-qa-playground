...
async function handleProxy(request: Request, { params }: { params: { path: string[] } }) {
  ...
  const targetUrl = `http://postgrest:3000/${pathStr}${new URL(request.url).search}`;

  const body = ['GET', 'HEAD'].includes(request.method) ? undefined : await request.text();
  const headers = new Headers(request.headers);
  headers.set('host', 'postgrest:3000');
  ...
  try {
    ...
    console.error('Server-side PostgREST proxy failed:', err);
